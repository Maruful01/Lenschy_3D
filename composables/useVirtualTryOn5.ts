...
// NOTE: PLANE_Z now only used as a “virtual face plane” reference, not for video
const PLANE_Z = -5;
...
const GLTF_TEMPLE_LEFT_NAME = "Cube.003"; // e.g. "Temple_L_End"
const GLTF_TEMPLE_RIGHT_NAME = "Cube.003"; // e.g. "Temple_R_End"
...
type GlassesUserData = {
  ...
  // OPTIONAL: here you can also store a face-occluder mesh if you implement it
};
...
export function useVirtualTryOn(
  ...
  // Keep a reference to the video texture so we can dispose when stopping
  ...
  const initThree = async () => {
    ...
    // --- NEW: use camera video as scene.background, NOT as PlaneGeometry ----
    if (videoEl) {
      videoTexture = new VideoTexture(videoEl);
      videoTexture.minFilter = THREE.LinearFilter;
      videoTexture.magFilter = THREE.LinearFilter;
      // Optional, depending on your Three.js version:
      // (videoTexture as any).colorSpace = THREE.SRGBColorSpace;
      scene.background = videoTexture; // background, no depth
    }

    // --- REMOVED: the video PlaneGeometry that was participating in depth ---
    // (We no longer add any plane for the background video.)
  ...
  const onResults = (results: any) => {
    ...
    const yawFactor = 1 + 0.5 * (yawClamped / maxYaw); // 1.0 → 1.2
    ...
    // OPTIONAL: debug temple ends – compute their world positions
    const { templeL, templeR } = obj.userData;
    if (templeL && templeR) {
      const worldL = templeL.getWorldPosition(new THREE.Vector3());
      const worldR = templeR.getWorldPosition(new THREE.Vector3());

      const earLWorld = toWorld3D(leftEar, vw, vh);
      earLWorld.z = positionZ;
      const earRWorld = toWorld3D(rightEar, vw, vh);
      earRWorld.z = positionZ;

      // console.log("TempleL vs EarL", worldL, earLWorld);
      // console.log("TempleR vs EarR", worldR, earRWorld);
    }
  ...
  const startCamera = async () => {
    ...
    if (!videoEl) {
      ...
      videoEl.style.display = "none"; // you can also append behind the canvas if needed
    ...
    // If renderer/scene already exist, but videoTexture was created before play():
    // ensure background texture is bound
    if (scene && !videoTexture && videoEl) {
      videoTexture = new VideoTexture(videoEl);
      videoTexture.minFilter = THREE.LinearFilter;
      videoTexture.magFilter = THREE.LinearFilter;
